...
class TestWorkflowIntegration:
    ...
    async def test_complete_workflow_flow(
        self, real_mysql_service, real_milvus_service, mock_llm_service
    ):
        """测试从异常发现到结果存储的完整流程."""
        ...
        with (
            patch(
                "oceanus_agent.workflow.graph.MySQLService",
                return_value=real_mysql_service,
            ),
            patch(
                "oceanus_agent.workflow.graph.MilvusService",
                return_value=real_milvus_service,
            ),
            patch(
                "oceanus_agent.workflow.graph.LLMService", return_value=mock_llm_service
            ),
        ):
            ...
            async with real_mysql_service.async_session() as session:
                res = await session.execute(
                    text("SELECT COUNT(*) FROM knowledge_cases")
                )
                count = res.scalar()
                assert count == 2
